...
namespace SocialPay.Core.Services
{
    public class SpectaSendPin : ISpectaSendPin
    {
        ...
        static readonly log4net.ILog _log4net = log4net.LogManager.GetLogger(typeof(SpectaSendPin));

        public SpectaSendPin(SocialPayDbContext context, ISpectaOnBoarding spectaOnboardingService, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
            ...
            try
            {
                using (var transaction = await _context.Database.BeginTransactionAsync())
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        _log4net.Error("Error occured" + " | " + "SendPin" + " | " + ex + " | " + DateTime.Now);
